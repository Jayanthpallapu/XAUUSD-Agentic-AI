...
@mcp.tool(
    ...
    description="Fetches the current spot price of Gold (XAUUSD) in USD.",
)
...
@mcp.tool(
    ...
    description="Fetches prices of commodities: Silver, WTI Crude Oil, Brent, Copper.",
)
...
@mcp.tool(
    ...
    description="Fetches global indices values: US Dollar Index (DXY), S&P 500, VIX.",
)
...
@mcp.tool(
    ...
    description="Fetches the current US 10-Year Treasury Yield.",
)
...
@mcp.tool(
    name="fetch_news_rss",
    description="Fetches latest financial RSS news feed. Query can be 'gold FOMC calendar'.",
)
...
@mcp.tool(
    ...
    description="Retrieves economic calendars highlighting NFP, FOMC, CPI events.",
)
...
@mcp.tool(
    ...
    description="Executes simulated paper trade signals. Direction must be BUY, SELL or HOLD.",
)
...
@mcp.tool(
    ...
    description="Fetches closed simulated trades logs and performance analytics statistics.",
)
...
@mcp.tool(
    ...
    description="Records educational feedback or corrective guidelines on a trade.",
)
...
def mcp_save_trade_journal(trade_data: str) -> str:
    return save_trade_journal(trade_data=trade_data)
...
def mcp_fetch_closed_trades(limit: int = 10) -> str:
    return fetch_closed_trades(limit=limit)


# 4. System Administration Tools
@mcp.tool(
    name="check_agent_health",
    description="Checks heartbeats, task counts, and uptime for all active agents.",
)
...
@mcp.tool(
    ...
    description="Restarts stuck or crashed agent nodes and resets error counts.",
)
...
@mcp.tool(
    ...
    description="Transmits warnings, signals or reports to Telegram.",
)
