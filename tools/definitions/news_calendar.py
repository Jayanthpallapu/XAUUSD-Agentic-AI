...
def fetch_news_rss(query: str = "gold price XAUUSD forex") -> str:
    ...
    try:
        ...
        for i, entry in enumerate(entries):
            ...
            bearish_words = ["fall", "plummet", "loss", "lower", "bearish", "selling", "slump", "drops", "strengthen", "hike", "down"]
...
def analyze_news_sentiment() -> str:
        ...
        try:
            ...
            if feed:
                ...
                for item in feed[:5]:
                    ...
                    headlines.append(f"- {item.get('title')} (Sentiment: {label}, Score: {score:.2f})")
                    
                avg_sentiment = sentiment_sum / len(feed[:5])
                overall_lbl = "Bullish" if avg_sentiment > 0.15 else "Bearish" if avg_sentiment < -0.15 else "Neutral"
    ...
    try:
        ...
        overall = "Bullish" if net_score > 0.1 else "Bearish" if net_score < -0.1 else "Neutral"
...
def fetch_economic_calendar() -> str:
    ...
    events_by_day = {
        "Monday": [
            {"event": "US ISM Manufacturing PMI", "impact": "High", "time": "14:00 UTC", "est": "48.2", "prev": "47.8"},
            {"event": "EUR ECB President Lagarde Speech", "impact": "Medium", "time": "15:30 UTC", "est": "N/A", "prev": "N/A"}
        ],
        "Tuesday": [
            {"event": "US JOLTs Job Openings", "impact": "High", "time": "14:00 UTC", "est": "8.10M", "prev": "8.06M"},
            {"event": "AUD RBA Interest Rate Decision", "impact": "High", "time": "04:30 UTC", "est": "4.35%", "prev": "4.35%"}
        ],
        "Wednesday": [
            {"event": "US ADP Non-Farm Employment Change", "impact": "High", "time": "12:15 UTC", "est": "150K", "prev": "152K"},
            {"event": "US ISM Services PMI", "impact": "High", "time": "14:00 UTC", "est": "50.8", "prev": "49.4"},
            {"event": "US FOMC Meeting Minutes", "impact": "High", "time": "18:00 UTC", "est": "N/A", "prev": "N/A"}
        ],
        "Thursday": [
            {"event": "US Unemployment Claims", "impact": "High", "time": "12:30 UTC", "est": "215K", "prev": "219K"},
            {"event": "EUR ECB Interest Rate Decision", "impact": "High", "time": "12:15 UTC", "est": "4.00%", "prev": "4.25%"}
        ],
        "Friday": [
            {"event": "US Non-Farm Payrolls (NFP)", "impact": "High", "time": "12:30 UTC", "est": "180K", "prev": "175K"},
            {"event": "US Unemployment Rate", "impact": "High", "time": "12:30 UTC", "est": "3.9%", "prev": "4.0%"}
        ]
    }
    
    events = events_by_day.get(day_name, [
        {"event": "No high-impact economic calendar events scheduled (Weekend)", "impact": "Low", "time": "N/A", "est": "N/A", "prev": "N/A"}
    ])
    
    calendar_output = f"Economic Calendar - {day_name} (Simulation based on typical schedules):\n\n"
    for ev in events:
        calendar_output += (
            f"- Event: {ev['event']}\n"
            f"  Time: {ev['time']}\n"
