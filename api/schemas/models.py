...
class EconomicEvent(BaseModel):
    ...
    expected_effect: str = Field(..., description="Expected bullish/bearish effect on gold")


class NewsResearchOutput(BaseModel):
    news_items: List[NewsItem] = Field(..., description="Latest gold-related news articles")
    economic_events: List[EconomicEvent] = Field(..., description="High-impact scheduled events today")
    fed_stance: str = Field(..., description="hawkish, dovish, or neutral based on recent Fed communications")
    inflation_data: str = Field(..., description="Latest CPI / inflation reading summary")
    is_high_impact_day: bool = Field(..., description="True if a major event (CPI, NFP, FOMC) is today")
    sentiment_score: float = Field(..., description="Net sentiment score from -1.0 (bearish) to +1.0 (bullish)")
    summary: str = Field(..., description="Detailed narrative summary of news findings")


class CorrelationPair(BaseModel):
    instrument: str = Field(..., description="Instrument name e.g. DXY, US10Y, BTC, VIX")
...
class CorrelationOutput(BaseModel):
    correlations: List[CorrelationPair] = Field(..., description="Correlated instrument analysis")
    ...
    confluence_score: float = Field(..., description="Overall confluence score 0.0 to 100.0")
...
class FundamentalDirectionOutput(BaseModel):
    ...
    key_drivers: List[str] = Field(..., description="Top 3-5 fundamental reasons for direction")
    risk_factors: List[str] = Field(..., description="Events or data that could invalidate the direction")
...
class TimeframeAnalysis(BaseModel):
    ...
    trend: str = Field(..., description="BULLISH, BEARISH, or RANGING")
    structure: str = Field(..., description="Market structure: HH/HL (uptrend), LH/LL (downtrend), or ranging")
    ...
    liquidity_zone: Optional[str] = Field(None, description="Identified liquidity pool zone")
    candle_pattern: Optional[str] = Field(None, description="Notable candle pattern if any")
    analysis: str = Field(..., description="Brief narrative of technical conditions on this timeframe")


class TechnicalDirectionOutput(BaseModel):
    direction: str = Field(..., description="BULLISH, BEARISH, or NEUTRAL")
    confidence: float = Field(..., description="Confidence score 0.0 to 1.0")
    timeframe_analyses: List[TimeframeAnalysis] = Field(..., description="Individual analysis per timeframe")
    htf_bias: str = Field(..., description="Higher timeframe bias (1W+1D combined): BULLISH, BEARISH, or NEUTRAL")
    ltf_entry_signal: str = Field(..., description="Lower timeframe entry signal: BUY, SELL, or WAIT")
    entry_zone: str = Field(..., description="Ideal entry zone e.g. '$2635 - $2640'")
    invalidation_level: float = Field(..., description="Price that invalidates the technical setup")
...
class QAIssue(BaseModel):
    issue_type: str = Field(..., description="confluence, rr_ratio, entry_quality, or risk")
...
class QATradeDecision(BaseModel):
    ...
    take_profit_1: float = Field(..., description="First take profit target (minimum RR 1:3)")
    take_profit_2: Optional[float] = Field(None, description="Optional second take profit target")
    lot_size: float = Field(..., description="Calculated lot size based on 1% account risk")
    risk_reward_ratio: float = Field(..., description="Actual RR ratio e.g. 3.5 means 1:3.5")
    account_risk_pct: float = Field(..., description="Percentage of account at risk e.g. 1.0")
    fundamental_confidence: float = Field(..., description="Fundamental direction confidence 0.0-1.0")
    technical_confidence: float = Field(..., description="Technical direction confidence 0.0-1.0")
    combined_confidence: float = Field(..., description="Combined weighted confidence score 0.0-1.0")
    issues_found: List[QAIssue] = Field(default_factory=list, description="List of issues detected")
...
class TradeAttribution(BaseModel):
    ...
    fundamental_factor: str = Field(..., description="How fundamental analysis contributed")
...
class LearningRecommendation(BaseModel):
    ...
    category: str = Field(..., description="entry_criteria, rr_requirements, timeframe_weights, confidence_thresholds")
    proposed_change: str = Field(..., description="Specific proposed change with rationale")
    supporting_evidence: str = Field(..., description="Trade data supporting this recommendation")
    expected_improvement: str = Field(..., description="Expected improvement in win rate or RR")
